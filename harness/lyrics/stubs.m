...
@interface SGHarnessPlaybackState : NSObject
@property (nonatomic, readonly) BOOL isPlaying, isPaused;
@end
@implementation SGHarnessPlaybackState
- (BOOL)isPlaying { return YES; }
- (BOOL)isPaused { return NO; }
@end

SPTPlayerState *SGRPlayerState(void) {
    static SGHarnessPlaybackState *state;
    if (!state) state = [SGHarnessPlaybackState new];
    return (SPTPlayerState *)state;
}
