...
- (instancetype)initWithLine:(SGKaraokeLine *)line width:(CGFloat)width style:(SGRKaraokeStyle *)style under:(SGRKaraokeLineView *)under blurred:(BOOL)blurred {
    ...
    NSMutableArray *texts = [NSMutableArray arrayWithObject:SGKaraokeLineText(line) ?: @""];
    if (style.pronunciation && line.pronunciation) [texts addObject:SGKaraokeLineText(line.pronunciation)];
    if (style.translation && line.translation.length) [texts addObject:line.translation];
    self.accessibilityLabel = [texts componentsJoinedByString:@", "];
...
- (SGRKaraokeStyle *)styleNow {
    return [[SGRKaraokeStyle alloc] initWithSize:_fontSize order:SGRLyricsTextOrder()
                                   pronunciation:_hasSpoken && SGFlag(SGRKeyLyricsPronunciation, SGRomanizedLyricsEnabled())
...
- (void)offerExtras {
    ...
    if (_hasSpoken) {
        BOOL on = SGFlag(SGRKeyLyricsPronunciation, SGRomanizedLyricsEnabled());
...
- (void)tick {
    ...
    NSArray *available = SGKaraokeLinesForTrack(track);
    if (!(track == _track || [track isEqualToString:_track]) || (_lines && available && _lines != available)) {
    ...
    CFTimeInterval shown = _link ? _link.targetTimestamp : CACurrentMediaTime();
    if (_dots.superview) [_dots showTime:now running:!_stillSince || shown - _stillSince < kStillFor at:shown];
}
