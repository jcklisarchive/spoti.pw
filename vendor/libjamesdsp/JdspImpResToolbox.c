...
static float *decompressedCoefficients = 0;
...
float* loadAudioFile(const char *filename, double targetFs, unsigned int *channels, drwav_uint64 *totalPCMFrameCount, int resampleQuality)
{
    ...
    // spoti.pw: the extension in any case, "IR.WAV" as well as "IR.wav".
    if (!strncasecmp(ext, "wav", 5) || !strncasecmp(ext, "irs", 5))
        pSampleData = drwav_open_file_and_read_pcm_frames_f32(filename, channels, &fs, totalPCMFrameCount, 0);
    if (!strncasecmp(ext, "flac", 5))
...
(const char* mIRFileName, int targetSampleRate, int* jImpInfo, int convMode, int* javaAdvSetPtr)
{
    ...
    // spoti.pw: both set, and the buffer checked, before they are read: a file that did not load left them
    // unset and went on to split a NULL buffer.
    unsigned int channels = 0;
    drwav_uint64 frameCount = 0;
    float *pFrameBuffer = loadAudioFile(mIRFileName, targetSampleRate, &channels, &frameCount, 1);
    if (!pFrameBuffer || channels == 0 || channels == 3 || channels > 4)
    {
        free(pFrameBuffer);
        return 0;
    }
    // spoti.pw: RootlessJamesDSP's check of the advanced settings (validateAdvImpParameter): a shift past
    // the buffer is replaced by the defaults rather than written out of bounds.
    {
        int frameCountGE8 = frameCount < 8 ? 8 : (int)frameCount;
        int splittedBufferSize = convMode == 2 ? (2 * frameCountGE8) : (int)frameCount;
        for (int k = 2; k < 6; k++)
        {
            if (javaAdvSetPtr[k] >= splittedBufferSize || javaAdvSetPtr[k] < 0)
            {
                javaAdvSetPtr[0] = -80;
                javaAdvSetPtr[1] = -100;
                javaAdvSetPtr[2] = javaAdvSetPtr[3] = javaAdvSetPtr[4] = javaAdvSetPtr[5] = 0;
                break;
            }
        }
    }
    ...
    if (convMode > 0)
    {
        ...
        else
        {
            ...
            // spoti.pw: a mono response spawns no thread, and th[-1] was written before the block.
            if (spawnNthread > 0)
