...
// spoti.pw: no background thread for the tail partitions. With it, the render thread took a mutex and could
// wait on a condition variable for a thread of normal priority every tail block; without it the tail is
// convolved inline, the single-threaded path the file already has.
// #define THREAD
#ifdef THREAD
