...
void JamesDSPInit(JamesDSPLib *jdsp, int n, float sample_rate)
{
	...
	//
	// spoti.pw: recursive, because JamesDSPSetSampleRate holds it while its forced refresh calls
	// CrossfeedEnable, ArbitraryResponseEqualizerEnable and StereoEnhancementRefresh, which take it again: a
	// plain mutex deadlocked there on the first rate change.
	pthread_mutexattr_t recursive;
	pthread_mutexattr_init(&recursive);
	pthread_mutexattr_settype(&recursive, PTHREAD_MUTEX_RECURSIVE);
	if (pthread_mutex_init(&jdsp->m_in_processing, &recursive) != 0)
